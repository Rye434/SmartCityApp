...
export const STORE_RAW_POSITION = "STORE_RAW_POSITION";
//endregion
...
export function preload(position) {
    return(dispatch) => {
        dispatch(storeRawPosition(position));
...
export function storeRawPosition(position) {
    return{
        type:STORE_RAW_POSITION,
...
// API calls
export function calculateDistance(position, requestList){
    console.log("CALL GOOGLE: DISTANCE");
    let url = 'https://maps.googleapis.com/maps/api/distancematrix/json?';
    let origin = JSON.stringify(position.coords.latitude)+','+JSON.stringify(position.coords.longitude);
    let destination = [];
    let key = Constants.MAPS_API_KEY_PLACES;
    for(let request in requestList.list) {
        let lat = [requestList.list[request].lat.split('.')[0] , requestList.list[request].lat.split('.')[1].slice(0,5)];
        let long = [requestList.list[request].long.split('.')[0] , requestList.list[request].long.split('.')[1].slice(0,5)];
        destination.push(lat.join('.')+","+long.join('.'))
    }

    destination = destination.join('|');
   // console.log(url+'origins='+origin+'&destinations='+destination+'&key='+key)
        return (dispatch)=>{
...
export function checkByPhone(phone, code, position) {
        ...
        axios.post(Constants.BASE_URL + '/registerservice/api/auth/checkPhoneRegister',
            ...
            .then((response) => {
                ...
                if(JSON.parse(response.request.response).errorCode == 82){
                    ...
                    console.log(code);
                    if(code != null){
                        dispatch(loginByPhone(phone,code, position))
                    }
                    dispatch((validateResponseCodeProfile(JSON.parse(response.request.response))))
...
//Login with Phone
export function loginByPhone(phone, encCode, position) {
        ...
        axios.post(Constants.BASE_URL + '/registerservice/api/auth/loginByPhone',
            ...
            .then((response) => {
                ...
                if(JSON.parse(response.request.response).errorCode == 0) {
                    ...
                    dispatch(getPersonalReqs(JSON.parse(response.request.response).userId, position))
...
export function getPersonalReqs(userId, position) {
...
export function toggleAck(userId, bool, id311, id, position) {
    ...
    return (dispatch)=>{
        axios.post(Constants.BASE_URL + '/registerservice/api/requests/acknowledgeRequest',
            ...
            .then((response) => {
                ...
                if(JSON.parse(response.request.response).errorCode === 0){
                            dispatch(fetchRequestList(position));
                            dispatch(getPersonalReqs(userId, position));
...
export function buildRequest(requestObj, userObj, position) {
    return (dispatch) => {
